...
Public Class FormKategori
    ...
    Sub judulgrid()
        ...
        dgv.Columns(0).HeaderText = "ID Kategori"
    ...
    Sub querytambah()
        Dim strsimpan As String = "INSERT INTO kategori" _
    ...
    Sub queryedit()
        Dim strEdit As String = "UPDATE kategori SET id_kategori = '" & TextBox1.Text.ToUpper & "',nama_kategori = '" & TextBox2.Text.ToUpper & "' " _
                              & "WHERE id_kategori = '" & id_data & "'"
        query(strEdit)
    ...
    Sub queryhapus()
        Dim strhapus As String = "DELETE FROM kategori WHERE id_kategori = '" & id_data & "'"
        query(strhapus)
        isigrid()
    ...
    Sub tambah()
        cmd = New MySqlCommand("SELECT * FROM kategori WHERE id_kategori = '" & TextBox1.Text & "'", konek)
        dr = cmd.ExecuteReader
        dr.Read()
        If dr.HasRows Then
            dr.Close()
            MsgBox("ID Kategori sudah ada!", 16, "Informasi")
            TextBox1.Focus()
        Else
            dr.Close()
            querytambah()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Dim nhps As Integer
        nhps = MsgBox("Yakin hapus kategori " & TextBox2.Text & " ?", 48 + 4 + 256, "Konfirmasi")
        If nhps = 6 Then
            queryhapus()
            reset()
        End If
