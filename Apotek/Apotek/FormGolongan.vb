...
Public Class FormGolongan
    ...
    Sub isigrid()
        Dim query As String = "SELECT * FROM golongan"
    ...
    Sub judulgrid()
        ...
        dgv.Columns(0).HeaderText = "ID Golongan"
    ...
    Sub querytambah()
        Dim strsimpan As String = "INSERT INTO golongan" _
    ...
    Sub queryedit()
        Dim strEdit As String = "UPDATE golongan SET id_golongan = '" & TextBox1.Text.ToUpper & "',nama_golongan = '" & TextBox2.Text.ToUpper & "' " _
                              & "WHERE id_golongan = '" & id_data & "'"
        query(strEdit)
    ...
    Sub queryhapus()
        Dim strhapus As String = "DELETE FROM golongan WHERE id_golongan = '" & id_data & "'"
        query(strhapus)
        isigrid()
    ...
    Sub tambah()
        cmd = New MySqlCommand("SELECT * FROM golongan WHERE id_golongan = '" & TextBox1.Text & "'", konek)
        dr = cmd.ExecuteReader
        dr.Read()
        If dr.HasRows Then
            dr.Close()
            MsgBox("ID Golongan sudah ada!", 16, "Informasi")
            TextBox1.Focus()
        Else
            dr.Close()
            querytambah()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Dim nhps As Integer
        nhps = MsgBox("Yakin hapus golongan " & TextBox2.Text & " ?", 48 + 4 + 256, "Konfirmasi")
        If nhps = 6 Then
            queryhapus()
            reset()
        End If
